...
import { EEmpanelmentType } from 'src/types/perspectiveProvider'
...
const LegalPendingVerifyDetailsList = (props: Props) => {
  ...
  const onSubmitHandle: SubmitHandler<any> = async (data) => {
    ...
    const res = await verifyEmpanelmentByLegal(setLoading, showToast, {
      remark: remark.length > 0 ? generateValidationMessage(remark as any) : '',
  ...
  const handleReturnToNetworkAndDc = async (status: string) => {
    const res = await verifyEmpanelmentByLegal(setLoading, showToast, {
      legalUnverifiedFields: checkedFields,
      remark:
            ...
            {state?.state?.verificationStatusByLegal === EVerificationProcess.Pending && (
              <>
                ...
                <div className='text-center items-center justify-center gap-5 flex pt-6'>
                  ...
                  {state?.state?.verificationStatusByLegal === EVerificationProcess.Pending &&
                    data?.childTicket?.empanelType === EEmpanelmentType?.Manual && (
                      <Button
                        color='mBlue'
                        sx={{
                          maxHeight: '35px',
                        ...
                        onClick={() => {
                          if (
                            ...
                            handleReturnToNetworkAndDc(EVerificationProcess?.ReturnedByLegal)
                          }
                        }}
                  ...
                  {state?.state?.verificationStatusByLegal === EVerificationProcess.Pending &&
                    data?.childTicket?.empanelType === EEmpanelmentType?.Auto && (
                      <Button
                        color='mBlue'
                        sx={{
                          maxHeight: '35px',
                        ...
                        onClick={() => {
                          if (
                            ...
                            handleReturnToNetworkAndDc(
                              EVerificationProcess?.ReturnedByLegalProvider,
                            )
                          }
                        }}
                        disabled={!(checkedFields?.length === 0) ? false : true}
