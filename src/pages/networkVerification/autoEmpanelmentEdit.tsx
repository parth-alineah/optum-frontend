...
import { EVerificationProcess, getOneEmpanelmentData } from '@/lib/empanelmentCountAndList'
import { editAutoEmpanelment } from '@/lib/autoEmpanelment'
import CheckInput from '@/components/CheckInput'
import { DateInput } from 'src/components/DateInput'
import Table from 'src/components/Table'
import { limitOfPage, TABLES } from 'src/utils/constants'
...
const AutoEditList = (props: Props) => {
        ...
        ?.map((s, index) => {
          if (s?.isAvailable) {
            return {
              ...
              otherQualification: '',
              otherCouncil: '',
              gender: s?.gender?._id ? s?.gender?._id : undefined,
            }
          }
        })
        .filter((x) => x && x),
  ...
  const checkLegalField = (field) => {
    if (
      data?.verificationStatusByLegal === EVerificationProcess.ReturnedByLegalProvider &&
      ...
      data?.verificationStatusByLegal === EVerificationProcess.Pending &&
      data?.verificationStatusByNw === EVerificationProcess.Pending &&
      data?.nwUnverifiedFields.length !== 0
    ) {
      return data?.nwUnverifiedFields && data?.nwUnverifiedFields.includes(field) ? true : false
    } else {
      return true
    }
  }
  ...
  useEffect(() => {
    if (
      ...
      for (const [section, fields] of Object.entries(fieldName)) {
        data?.nwUnverifiedFields?.forEach((shortName: string) => {
          if (fields.includes(shortName)) {
            // @ts-expect-error: Ignore type checking for this line
            setError(`${section}.${shortName}`, {
              type: 'manual',
  ...
  }, [data?.nwUnverifiedFields])

  const handleFieldChange = (section: string, fieldName: string) => {
    if (data?.verificationStatusByLegal === EVerificationProcess.ReturnedByLegalProvider) {
      //@ts-expect-error: Ignore type checking for this line
